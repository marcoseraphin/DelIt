...
namespace delt.ContentViews
{
	public partial class ShowItemCV : ContentView
	{
		...
		public static readonly BindableProperty ShowClassificationProperty = BindableProperty.Create(
			...
			typeof(string),
			typeof(ShowItemCV),
			propertyChanged: ShowClassificationPropertyChanged);
		...
		private void UpdateCountryImage(string countryCode)
		{
			...
			{
				case "de-DE":
					InnerCountrySvgImage.Source = "066-germany.svg";
					break;
		...
		private static void ShowClassificationPropertyChanged(BindableObject bindable, object oldValue, object newValue)
		{
			if ((bindable is ShowItemCV showItem) && (newValue is String showClassification))
			{
				showItem.UpdateClassificationLabelVisibility(showClassification);
			}

		}


		private void UpdateClassificationLabelVisibility(string showClassification)
		{
			if (String.IsNullOrEmpty(showClassification))
			{
				ShowClassificationLabel.IsVisible = false;
			}
			else
			{
				ShowClassificationLabel.IsVisible = true;
			}
		}
